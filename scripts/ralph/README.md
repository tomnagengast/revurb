...
- `sync.sh`: this is what happens with each turn. This is where we get a little creative:
...
- `current.md`: this allows us to gently guide ralph. The core prompt are intentionally vauge and this file let's up nudge it a direction the `reviewer` might not have done other wise.
